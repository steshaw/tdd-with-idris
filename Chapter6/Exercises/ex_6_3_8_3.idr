...
parsePortion SString input = getQuoted (unpack input)
  where
...
parsePortion SInt input = case span isDigit input of
                                   ("", rest) => Nothing
                                   (num, rest) => Just (cast num, ltrim rest)
parsePortion SChar input = case unpack input of
                                (c :: cs) => Just (c, ltrim (pack cs))
                                [] => Nothing
parsePortion (schemal .+. schemar) input
    = do (l_val, input') <- parsePortion schemal input
         (r_val, input'') <- parsePortion schemar input'
